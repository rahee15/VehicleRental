    ...
    public void executeCommand(String[] operands) {
        try {
            ...
            if (branchOptional.isEmpty()) {
                ...
                List<BranchVehicle> branchVehicleList = branchVehicleService
                        .findBranchVehicleByBranch(branchOptional.get());
                ...
                List<Booking> bookingList = bookingService.finaAllBookingBetweenStartAndEndHour(
                        branchOptional.get(), bookingStartTime,
                        bookingEndTime);
                ...
                if (bookingList.size() == branchVehicleList.size()) {
                    ...
                    Branch carBranch = Branch.builder().branchType(branchType).vehicleType(VehicleType.CAR).build();

                    //If 80% Cars are booked in a given branch then increase price by 10%
                    List<Booking> carBookingList = bookingService.finaAllBookingBetweenStartAndEndHour(
                            carBranch, bookingStartTime,
                            bookingEndTime);

                    boolean shouldIncreasePrice = ((double)carBookingList.size()) >= ((0.80 * (double)branchVehicleService.findBranchVehicleByBranch(carBranch).size()));


                    Booking booking = bookingService.createBooking(branchVehicleList.get(bookingList.size()),
                            bookingStartTime, bookingEndTime);

                    double finalPrice = booking.getBranchVehicle().getPrice() + (shouldIncreasePrice ? 0.10 * booking.getBranchVehicle().getPrice() : 0);
